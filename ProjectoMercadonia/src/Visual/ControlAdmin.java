...
public class ControlAdmin extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ControlAdmin frame = new ControlAdmin();
	...
	public ControlAdmin() {
		...
		panel.setBounds(95, 117, 500, 338);
		fondo.add(panel);
		...
		btnPedidos.setFont(new Font("Roboto", Font.BOLD, 20));
		panel.add(btnPedidos, BorderLayout.WEST);
		...
		btnUsuarios.setFont(new Font("Roboto", Font.BOLD, 20));
		panel.add(btnUsuarios, BorderLayout.CENTER);
		
		JButton btnProductos = new JButton("Productos");
		btnProductos.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				ProductosAdmin obj=new ProductosAdmin();
				obj.setVisible(true);
				dispose();
			}
		});
		btnProductos.setFont(new Font("Roboto", Font.BOLD, 20));
		panel.add(btnProductos, BorderLayout.EAST);
	}
}
